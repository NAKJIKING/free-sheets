...
- `check_outputs.py` — **수집 결과물 타당성 검사 (수집 뒤 항상 실행)**.
  파일이 '있는지'가 아니라 '말이 되는지' 본다 — 미디에 음표가 실제로 들어 있는지,
  악보가 진짜 PDF 인지, **쪽당 음표 수가 같은 출처 안에서 지나치게 적지 않은지**.
  문제가 있으면 종료코드 1.
  → 2026-09 에 thesession 2,659곡이 첫 줄만 담긴 채 몇 달간 배포된 사고가 있었고,
    당시 유일한 검사가 "미디가 60바이트보다 큰가"라 그냥 통과했다. 그래서 만들었다.
- `regen_session.py` — thesession 악보·미디 재생성(복구용, 멱등, `--ids` 로 일부만)
